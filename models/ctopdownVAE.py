...
class ConditionalTopDownVAE(nn.Module):
    def __init__(self, x_dim, y_dim, h1_dim=64, h2_dim=64, e_dim=64, ze_dim=64, z1_dim=64, z2_dim=64, hid_dim=64, n_layers=3, activation='relu'):
        ...
        self.pn_x_h1 = PointNet(x_dim, h1_dim, hid_dim=hid_dim, n_layers=n_layers, activation=activation)
        self.pn_h1_h2 = PointNet(h1_dim, h2_dim, hid_dim=hid_dim, n_layers=n_layers, activation=activation)
        self.pn_h2_z2 = PointNet(h2_dim, 2 * z2_dim, hid_dim=hid_dim, n_layers=n_layers, activation=activation)
        self.cpn_z2_z1 = CPointNet(z2_dim, 2 * z1_dim, ze_dim, hid_dim=hid_dim, n_layers=n_layers, activation=activation)
        self.pn_h1_z1 = PointNet(h1_dim, 2 * z1_dim, hid_dim=hid_dim, n_layers=n_layers, activation=activation)
        self.pn_z1_x = PointNet(z1_dim, 2 * x_dim, hid_dim=hid_dim, n_layers=n_layers, activation=activation)

        self.mlp_h2_e = MLP(h2_dim, e_dim, hid_dim=hid_dim, n_layers=n_layers, activation=activation)
        self.mlp_e_ze = MLP(e_dim, 2 * ze_dim, hid_dim=hid_dim, n_layers=n_layers, activation=activation)
    ...
    def forward(self, x, only_classify=False, epoch=None):
        ...
        mu_x, logvar_x = self.pn_z1_x(z1).chunk(2, 1)
        ...
        return elbo, nll.mean(), kl_z1.mean(), kl_z2.mean(), kl_ze.mean()
...
if __name__ == '__main__':
    ...
    hvae = ConditionalTopDownVAE(2, 4, h1_dim=32, h2_dim=34, e_dim=64, z1_dim=128, z2_dim=256).to(device)
